...
function findHostSdk() {
  ...
  // Heuristic: try to find any object with actions.composeCast
  try {
    for (const key of Object.keys(window).slice(0, 200)) {
      try {
        ...
        if (
          ...
          typeof val === 'object' &&
          val.actions &&
          typeof val.actions.composeCast === 'function'
        ) {
  ...
  safeLog('sdkShim: no host SDK found; will use fallback shim');
  return null;
}
